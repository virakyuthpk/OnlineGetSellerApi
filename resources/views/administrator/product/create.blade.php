...
@endif
<form class="form-horizontal" id="frm-prouduct" method="post" action="{{route('post-product',$id)}}" enctype="multipart/form-data">
    {{csrf_field()}}
            ...
            <div class="panel-body">
                    ...
                    <select class="form-control" name="category">
                        @if(!$category->isEmpty())
                            ...
                            @foreach($category as $cate)
                                <option value="{!!$cate->id!!}" {!!$product?$cate->id==$product->category_id ?'selected':'':''!!}>{!!$cate->title_en!!}</option>
                            @endforeach
                        @endif
                    </select>
                </div>
                    ...
                    <select class="form-control" name="subcat">
                        ...
                        @if($id != null)
                            @foreach($subcate as $sub)
                                <option value="{!!$sub->id!!}" {!!$sub?$sub->id==$product->parent_category?'selected':'':''!!}>
                                {!!$sub->title_en!!}
                                </option>
                            @endforeach
                        @endif
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" name="child">
                        @if($id != null)
                            @foreach($child as $ch)
                                <option value="{!!$ch->id!!}" {!!$product?$ch->	id==$product->sub_id?'selected':'':''!!}>
                                {!!$ch->title_en!!}
                                </option>
                            @endforeach
                        @endif
...
<div class="col-md-12">
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            @include('administrator.product.tab.gallery')
        </div>
    </div>
</div>
@stop
    ...
    <script type="text/javascript">
       $(document).ready(function(){
                ...
                if(cateid) {
                    $.ajax({
                        ...
                        success:function(data) {
                            ...
                            $('select[name="subcat"]').append('<option value="0">Select parent category</option>');
            ...
            $('select[name="subcat"]').on('change', function() {
                ...
                if(child) {
                    $.ajax({
                        url: '/admin/child-category/'+child,
                        type: "get",
                        dataType: "json",
                        success:function(data) {
                            ...
                            if(data.length > 0){
                                $.each(data, function(key, value) {
                                $('select[name="child"]').append('<option value="'+ value.id +'">'+ value.title_en +'</option>');
                            });
                            }else{
                                $('select[name="subcat"]').append('<option value="0">No Child</option>');
                            }
            ...
            $("#frm-prouduct").submit(function(e) {
                ...
                messages: {
                    ...
                    subcat:'Please Select parent category',
                    child:'Please Select child category',
                    image: {
        ...
        $(".drag-img").on('drop', function (e){
            ...
            $.ajax({
                url: "{{route('admin-photos-product')}}",
                headers: {
                    'X-CSRF-TOKEN': token
                },
                type: "POST",
