...
class ApiController extends Controller
{
    ...
    public function popular(){
    	try {
           $data['product'] = Orders::select('orders.product_id','products.name_en','products.image','products.id','products.sell_price')
            ->join('products', 'products.id', 'orders.product_id')
            ->groupBy('orders.product_id','products.name_en', 'products.image','products.id','products.sell_price')
            ->take(4)
            ->get();
            foreach ($data['product'] as  $pro) {
                if ($pro->image == "1") {
    ...
    {
        try {
             $data['product'] = Product::where('parent_category', $id)->where('status',1)->get();
             foreach ($data['product'] as  $pro) {
              /*  $pro->supplier = Supplier::where('id',$pro->supplier_id)->first();
                $pro->unit = Unit::where('id',$pro->unit_id)->first();
                $pro->category = Category::where('id',$pro->category_id)->first();
    ...
    {
        try {
            $data = Category::where('status',1)->inRandomOrder()->get();
    ...
    public function categoryHome(){
        try {
            $data = Category::where('parent_id', '!=', 0)->where('status',1)->inRandomOrder()->take(9)->get();
             foreach ($data as  $c) {
                $c->icon = $c->icon ? $this->URL . '/uploads/icon/' . $c->icon : $this->URL . '/uploads/default-img.jpg';
             }
            return response()->json($data);
    ...
    public function OrderNow($id){
       try {
            ...
            $district = District::get();
             $variant = Product_varaint::where('product_id',$id)->select('varaint_id')->get();
            dd($variant);
            //       $total_price = '';
            $total_price = '';
    ...
    public function listVendor(){
        try {
            $data = Vendors::where('status', '1')->inRandomOrder()->take(6)->get();
